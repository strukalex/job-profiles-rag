...
async def handle_draw_pr_graph(
    ...
    # # Group data by department and status, then sum views
    # dept_status_views = df.groupby(['department', 'state'])['views'].sum()

    # # Unstack the dataframe to have departments as columns and status as rows
    # dept_status_views_unstacked = dept_status_views.unstack(fill_value=0)

    # # Create stacked bar chart
    # dept_status_views_unstacked.plot(kind='bar', stacked=True)

    # # Add title and labels
    # plt.title('Status Distribution Across Departments')
